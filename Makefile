...
update:
	-(LMSLATEST=`wget -O - -q "http://www.mysqueezebox.com/update/?version=7.9.1&revision=1&geturl=1&os=deb"`; if [ "`cat lmsdeb.txt`" = "$$LMSLATEST" ]; then echo "No update available"; exit 1; else /bin/echo -n $$LMSLATEST > lmsdeb.txt; fi)
